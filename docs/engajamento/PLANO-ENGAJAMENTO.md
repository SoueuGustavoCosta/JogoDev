...
| Etapa 4 | Conferir o `output` | **Na Etapa 5:** teste que roda o `code` de cada pergunta `kind: 'output'` de PHP no PHP do laboratório e confere a opção certa. Python e Java não rodam no navegador: gerar uma lista dessas perguntas (ilha, módulo, id, código, resposta) para o autor conferir à mão. |
