...
| Etapa 3 | Pergunta antes da explicação | Como as perguntas mantêm a ordem original, alguma pode cair antes da tela que explica o assunto. **Decisão do autor: na Etapa 5**, adicionar em cada pergunta um campo opcional dizendo depois de qual bloco ela deve aparecer (ex.: `after: <posição ou id do bloco>`), respeitado por `paginateModule`. **Feito na Etapa 5:** campo `afterBlock` (posição do bloco, começando em 0). |
| Etapa 3 | Contador "N seguidas" | Decisão do autor: conta só acertos de primeira. Manter. |
| Etapa 3.5 | Ids das perguntas | Toda pergunta tem `id` (`q1`..`qN`, na ordem da Etapa 3.5). **Regra para as Etapas 4 e 5:** ao converter ou mover uma pergunta, ela leva o id junto; pergunta nova ganha o próximo id livre (`q{N+1}`). Nunca reaproveitar id. `src/content/quizIds.test.ts` impede que um id publicado suma. |
| Etapa 3.5 | Versão antiga do app lendo o progresso por id | O app antigo (anterior à Etapa 3.5) não entende `quizResults` por id, e o merge dele descarta as chaves por id quando o mesmo módulo existe nas duas cópias (aba antiga aberta durante o deploy fazendo backup, ou rollback). **Decisão do autor (2026-09-26): cópia extra.** Toda gravação guarda também os resultados por id na raiz do progresso (`quizBackup`), que o merge antigo preserva; toda leitura devolve aos módulos o que só estiver na cópia. Testado com o merge antigo de verdade (`src/domain/progress/__fixtures__/mergeBeforeQuizIds.ts`). Enquanto a cópia existir, a aba antiga ainda mostra menos XP de quiz até recarregar (só na tela; nada se perde). Rollback continua não recomendado: prefira um commit novo. |
| Etapa 3.5 | **TODO(autor): remover a cópia extra `quizBackup` a partir de 2026-10-24** (4 semanas depois da Etapa 3.5) | Apagar `src/domain/progress/quizBackup.ts` (+ teste), o campo `quizBackup` em `types.ts`, a linha em `merge.ts`, as chamadas em `application/usecases/quizIdMigration.ts` e a pasta `src/domain/progress/__fixtures__/`. Progresso salvo com o campo continua válido (campo opcional ignorado); se quiser limpar, remover a chave ao gravar. |
| Etapa 4 | Conferir o `output` | **Feito na Etapa 5:** teste que roda o `code` de cada pergunta `kind: 'output'` de PHP no PHP do laboratório e confere a opção certa. Python e Java não rodam no navegador: gerar uma lista dessas perguntas (ilha, módulo, id, código, resposta) para o autor conferir à mão. Lista em `docs/engajamento/conferir-output.md` (8 de Python, 7 de Java); SQL também é conferido automaticamente (PGlite). |
| Etapa 5 | **TODO(autor): conferir à mão as 15 saídas de Python e Java** | Marcar cada linha de `docs/engajamento/conferir-output.md`. Todas já foram rodadas pelo Claude Code com `python3`/`java` locais e bateram. |
| Etapa 5 | Resumo da conversão | Tabelas antes/depois de cada ilha em `docs/engajamento/etapa-5-conversao.md`. Todas as 6 ilhas abaixo de 40% de alternativas. |
| Etapa 6 | Laboratório dentro da lição | Feito: bloco `try` (SQL, PHP, Git) com pilotos em Banco de Dados (WHERE, JOIN) e Git (Ciclo). Bundle: entrada +1 KB (565.437 → 566.523 bytes), JS total +9 KB; PGlite e PHP continuam fora do carregamento inicial e só baixam quando a tela `try` aparece. Próximo passo sugerido (não feito): mais blocos `try`, inclusive de PHP na Lógica. |
| Etapa 7 | SQL da Anomalia do Dia: **aplicado em 2026-09-26** (pelo Claude Code, a pedido do autor) | Rodar `supabase/2026-09-26_anomalias_resolvidas.sql` no SQL Editor do Supabase (tabela `anomalias_resolvidas` + função `contar_anomalias_resolvidas`). Até lá o app funciona igual, só sem o contador "N viajantes já consertaram". Depois de aplicar, mover o conteúdo para `supabase/schema.sql`. |
| Etapa 7 | Anomalia acima de Base em era não aberta | Decisão tomada sem o autor (regra ambígua no plano): a anomalia do dia é a mesma para todos; se ela for de nível acima de Base numa era que o viajante ainda não abriu, ele recebe a "reserva Base" do dia (também igual para todos nessa situação). Hoje 28 das 30 são Base. |
| Etapa 7 | Linha do tempo e eventos no Início | O mockup do Início também mostra a Linha do Tempo (Etapa 8) e o evento ativo (Etapa 11): entram nessas etapas. |
| Etapa 8 | Bug antigo corrigido | Ao concluir a última lição de uma ilha, a gravação do troféu apagava a insígnia do módulo recém-ganha (existe também na `main`). Corrigido nesta branch (commit "Corrige: insígnia do último módulo sumia ao concluir a ilha"). Quem perdeu a insígnia assim a recupera concluindo de novo? Não: o módulo já está concluído. TODO(autor): decidir se vale uma rotina que reconceda insígnias de módulos já concluídos. |
| Etapa 8 | Linha do Tempo: decisões tomadas | Perdeu mais de um dia: usa uma âncora por dia perdido, se tiver todas; senão ramifica. Ramificar zera a sequência (o próximo dia jogado é o dia 1), guarda o recorde e move o Eco uma era no mapa (fica na última era se passar do fim). O Eco aparece desde o começo, na primeira era. |
| Etapa 9 | SQL dos cosméticos: **aplicado em 2026-09-26** | Rodar `supabase/2026-09-26_cosmeticos.sql` no SQL Editor do Supabase (coluna `jogadores.cosmeticos` + `grant select` dessa coluna). Conferido contra o banco real (só leitura). Até lá o visual fica salvo só no aparelho (e no backup do progresso) e os avatares dos outros aparecem sem cosméticos. Depois de aplicar, mover o conteúdo para `supabase/schema.sql`. |
| Etapa 9 | Fragmentos e loja: decisões tomadas | Saldo = histórico em ordem de data, nunca abaixo de 0: se dois aparelhos gastaram o mesmo saldo antes de sincronizar, o viajante fica com os dois itens e saldo 0 (o gasto a mais é perdoado, não vira dívida). Comprar o mesmo item em dois aparelhos cobra uma vez (id `buy:<item>`). Os ganhos da anomalia saem de `anomalies` (id `anomaly:<dia>`), sem cópia. Preços: comum 20–30 ◆, raro 60–80 ◆, lendário 180–200 ◆ (a anomalia dá 10 ◆/dia). Itens só de evento: Fenda do Eco (Eco Solto), Convergência e Selo da semana (Liga); a entrega deles entra nas Etapas 10 e 11 (`grantItem` já existe). |
| Etapa 10 | SQL da Liga: **aplicado em 2026-09-26**; **TODO(autor): confirmar o teto** | Rodar `supabase/2026-09-26_xp_semanal.sql` (tabela `xp_semanal` + funções `semana_atual`, `registrar_xp_semanal`, `liga_da_semana`). Conferido contra o banco real (só leitura) e testado num Postgres local (PGlite). Teto provisório: 10000 XP por semana (`v_teto` na função): TODO(autor) confirmar. Até aplicar, a aba Liga mostra só o próprio viajante com o aviso "fora do ar". Depois, mover para `supabase/schema.sql`. |
| Etapa 10 | Liga: decisões tomadas | Tabela (não view): o XP é calculado no app e não há eventos de XP no banco; uma linha por viajante por semana gasta pouco do plano gratuito. O servidor guarda o maior total visto e só aceita a semana atual (XP enviado nos últimos minutos de domingo depois da virada é perdido). O XP da semana no aparelho fica por fonte (`quiz:…`, `module:…`, anomalias pela data), as últimas 3 semanas; o envio acompanha o batimento de presença (~90 s), a conclusão de módulo e a anomalia. Selo: ao abrir a Liga, quem ficou no top 3 da semana anterior (com XP > 0) ganha o item "Selo da semana" (uma vez por semana, guardado em `leagueSeals`). A aba Liga saiu do Hall; o Hall continua em /hall, com link no fim da Liga. |
| Etapa 11 | SQL da Convergência: **aplicado em 2026-09-26** | Rodar `supabase/2026-09-26_convergencia.sql` (função `contar_anomalias_periodo`), **depois** do SQL da Etapa 7 (tabela `anomalias_resolvidas`). Testado num Postgres local (PGlite). Até lá o card da Convergência mostra a meta sem a contagem e ninguém ganha o prêmio. Depois, mover para `supabase/schema.sql`. |
| Etapa 11 | Eventos: decisões tomadas | Surto: o XP a mais fica em `xpBonus` (por fonte, união por id) e entra no total, no nível e na Liga; a anomalia grava o XP já multiplicado; Surtos no mesmo dia não somam (vale o maior). Eco Solto: 3 rodadas sorteadas por dia (iguais para todos) de 9, todas de tocar, com o motor do chefe de fase; 1ª vitória dá a "Fenda do Eco" (raro), as seguintes +15 ◆ uma vez por dia; perder não tira nada. Convergência de outubro/2026: 100 anomalias da turma liberam a cor "Convergência"; o prêmio chega para cada viajante quando ele abre o Início ou a Liga depois da meta batida. Como criar eventos: CLAUDE.md, seção 10. |
| Etapa 13A | Oficina: decisões tomadas | Linguagens: PHP (mesmo php-wasm do laboratório, recriado a cada teste com `refresh()` para função declarada não dar "Cannot redeclare") e JS (Web Worker novo por teste, sem rede nem armazenamento, encerrado em 2 s). PHP não respeita `set_time_limit` no WebAssembly: a guarda de tempo usa `declare(ticks=1)`; um laço com o corpo totalmente vazio (`while(true);`) não gera tick e ainda pode travar a aba até recarregar. **TODO(autor): Python com Pyodide sob demanda** (o botão aparece como "em breve"). Comparação de saída: ignora espaços/linhas em branco no fim, maiúsculas e acentos ("impar" = "ímpar"), e aceita 5 = 5.0 e vírgula decimal. Depois de um teste que estoura o tempo, os seguintes não rodam. A oficina aparece na ilha depois do módulo relacionado (`after`) e no Início (card "Oficina do Viajante"). |
| Etapa 13B | SQL da Oficina: **aplicado em 2026-09-26** | Rodar `supabase/2026-09-26_oficina.sql` (tabelas `solucoes_oficina`, `estrelas_solucao`, `oficinas_resolvidas` + funções `mural_oficina` e `contar_oficinas_periodo`). Conferido contra o banco real (só leitura) e testado num Postgres local. Até lá o mural aparece "fora do ar" e a Convergência por oficinas não conta. Depois, mover para `supabase/schema.sql`. |
| Etapa 13B | Oficina: decisões tomadas | Modo blocos: paleta por oficina e linguagem em `content/workshops` (linhas reais, com peças de caminhos diferentes e peças que não servem); `[[valor]]` marca o trecho editável; as chaves controlam a indentação. Arrastar pela alça ⋮⋮ (ou setas do teclado); segurar (ou toque duplo) edita; ✕ tira. Texto → blocos só se cada linha couber na paleta (comentários e `<?php` são ignorados). Mural: só abre depois de resolver; publicar pede confirmação (nome, foto e código ficam públicos); limite de 4000 caracteres e filtro básico de palavrões no app (TODO(autor): ampliar a lista em `domain/workshop/mural.ts` se precisar); estrela nunca na própria solução (RLS). Eventos: o Eco Solto pode indicar uma oficina (`workshopId`, hoje a Mini calculadora) e a Convergência pode contar oficinas (`metric: 'oficinas'`). Publicar não dá XP. |
| Etapa 12A | PWA: o que foi feito (parar para o autor testar) | Antes havia só o manifest, sem ícones nem service worker. Agora: ícones originais (a Sintaxe na plataforma; 192, 512, maskable e apple-touch), manifest completo, metas do iPhone, e `sw.js` gerado no build (`pwa/precache.ts` + `pwa/service-worker.js`) com ~70 arquivos do app (~1 MB) guardados no primeiro acesso: lições, quizzes, mapa e oficinas abrem offline. PGlite e PHP (vários MB) não entram: ficam guardados só depois da primeira vez que o laboratório abre. Nada de Supabase/métricas no cache. O service worker só registra em produção. Tela "Adicionar à tela inicial" (Android com botão nativo quando o Chrome deixa; iPhone com o passo a passo do Safari) aparece sozinha uma vez, logo depois da primeira anomalia consertada (evento `install_prompt_shown`), e fica sempre disponível na aba Viajante ("Instalar o app"). **Como testar:** abrir o preview da Vercel no celular, navegar um pouco, ativar o modo avião e abrir uma lição. **Parte B (lembretes com Web Push) não foi feita:** o plano pede para parar aqui até o autor aprovar a A, e ela depende das chaves VAPID e de uma Edge Function agendada no Supabase. |
