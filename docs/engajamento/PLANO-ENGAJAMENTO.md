...
| Etapa 3.5 | Versão antiga do app lendo o progresso por id | O app antigo (anterior à Etapa 3.5) não entende `quizResults` por id. **Aba antiga aberta durante o deploy:** mostra menos XP de quiz até recarregar; e o backup periódico dela (a cada batimento de presença) usa o merge antigo, que **descarta os resultados por id** quando o mesmo módulo existe nas duas cópias (aparelho e nuvem). Módulos concluídos, bônus de conclusão, insígnias, missões e sequência não são afetados. Só acontece se, com a aba antiga ainda aberta, houver progresso já no formato novo no mesmo navegador (outra aba nova) ou na nuvem (outro aparelho já atualizado). Some ao recarregar a aba (não há service worker). **Rollback:** nunca voltar a Vercel para um deploy anterior à Etapa 3.5 depois de publicá-la (o app antigo faria esse descarte em todo backup); se precisar desfazer algo, fazer um commit novo. Não dá para corrigir em código já publicado; risco aceito pelo autor? **TODO(autor): confirmar.** Alternativa, se quiser: uma cópia extra dos resultados na raiz do progresso (o merge antigo preserva campos da raiz), removida numa etapa futura. |
