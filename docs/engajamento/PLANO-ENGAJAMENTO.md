...
| Etapa 3.5 | Aba antiga aberta no deploy | Uma aba aberta com a versão antiga do app (sem recarregar) depois do deploy não entende o progresso por id: mostra menos XP de quiz até recarregar, e se ela fizer backup na nuvem nesse meio-tempo, o merge antigo descarta os resultados por id **da cópia da nuvem** (módulos concluídos, bônus, insígnias e o progresso do aparelho novo ficam; o aparelho novo sobe tudo de volta no próximo backup). Não há service worker, então some ao recarregar. Não dá para corrigir em código já publicado; risco aceito e registrado. |
