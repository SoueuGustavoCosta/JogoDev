...
| Etapa 7 | **TODO(autor): aplicar o SQL da Anomalia do Dia** | Rodar `supabase/2026-09-26_anomalias_resolvidas.sql` no SQL Editor do Supabase (tabela `anomalias_resolvidas` + função `contar_anomalias_resolvidas`). Até lá o app funciona igual, só sem o contador "N viajantes já consertaram". Depois de aplicar, mover o conteúdo para `supabase/schema.sql`. |
...
| Etapa 9 | **TODO(autor): aplicar o SQL dos cosméticos** | Rodar `supabase/2026-09-26_cosmeticos.sql` no SQL Editor do Supabase (coluna `jogadores.cosmeticos` + `grant select` dessa coluna). Conferido contra o banco real (só leitura). Até lá o visual fica salvo só no aparelho (e no backup do progresso) e os avatares dos outros aparecem sem cosméticos. Depois de aplicar, mover o conteúdo para `supabase/schema.sql`. |
...
| Etapa 11 | **TODO(autor): aplicar o SQL da Convergência** | Rodar `supabase/2026-09-26_convergencia.sql` (função `contar_anomalias_periodo`), **depois** do SQL da Etapa 7 (tabela `anomalias_resolvidas`). Testado num Postgres local (PGlite). Até lá o card da Convergência mostra a meta sem a contagem e ninguém ganha o prêmio. Depois, mover para `supabase/schema.sql`. |
