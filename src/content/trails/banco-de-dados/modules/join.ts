...
export const modJoin: Module = {
  ...
  blocks: [
    ...
    { t: 'note', k: "Como lembrar", x: "<b>INNER JOIN</b> traz só o que combina dos dois lados. <b>LEFT JOIN</b> traz tudo da tabela da esquerda (<code>FROM</code>) e completa com NULL onde não achou par. As letras <code>c</code> e <code>p</code> são apelidos de tabela, para escrever menos." },
    {
      ...
      engine: 'sql',
      file: "loja.sql",
      brief: "Liste o nome de cada cliente ao lado do total de cada pedido dele",
      starter: "SELECT c.nome, p.total\nFROM clientes c\n",
      hint: "Falta o JOIN: JOIN pedidos p ON p.cliente_id = c.id",
      ds: 'loja',
      solution: "SELECT c.nome, p.total FROM clientes c JOIN pedidos p ON p.cliente_id = c.id;",
    },
