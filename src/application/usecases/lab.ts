...
/** Como conferir um SQL: igual a uma consulta de referência, ou pelo estado final do banco. */
export type SqlCheck =
  | { kind: 'select'; solution: string; ordered?: boolean }
  | { kind: 'state'; verify: string; expect: string[][] };

/**
 * Confere o resultado de um SQL já executado (`blocks`, no banco em que ele rodou) contra a
 * regra das missões: kind "select" compara a última tabela com a consulta de referência;
 * kind "state" roda `verify` e compara com `expect`. Mesma regra do protótipo original,
 * usada pelas missões do laboratório e pelo laboratório dentro da lição (bloco `try`).
 */
export async function checkSqlResult(
  ...
  if (check.kind === 'select') {
    ...
    const expected = await engine.query(check.solution);
    ...
    const result = await engine.query(check.verify);
    gotRows = result.rows;
    expectedRows = check.expect;
  }

  let a = gotRows.map(stringifyRow);
  let b = expectedRows.map(stringifyRow);
  if (check.kind === 'select' && !check.ordered) {
    a = [...a].sort();
    b = [...b].sort();
  }
  ...
  if (!same) {
    return {
      ...
      message:
        check.kind === 'select'
          ? `As linhas não batem com o esperado. Confira os valores, os filtros${check.ordered ? ' e a ordem.' : '.'}`
...
export async function verifyMission(
  ...
  const verdict = await checkSqlResult(deps.engine, mission, blocks);
